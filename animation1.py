...
class main(Scene):
    def construct(self):
        ...
        noisy_graph = ax.plot(noisy, x_range=[-3, 3], use_smoothing=True).set_color(
            RED_B
        )
        ...
        y_label = Text("Pressure").shift(UP * 3).scale(1)
        x_label = Text("Time").shift(RIGHT * 3.35).scale(1)

        self.play(Create(ax), Create(box), run_time=0.5)
        ...
        filtered_graph = ax.plot(
            filtered, x_range=[-3, 3], use_smoothing=True
        ).set_color(GREEN_B)
