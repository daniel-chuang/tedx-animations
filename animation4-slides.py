    ...
    def construct(self):
        ...
        for i in range(53):
            ...
            if i == 2:
                self.add(circle3)
            self.play(
                f.animate.set_value(f.get_value() - 2),
                run_time=51 / 53,
                rate_func=linear,
            )
...
def freq1(t):
    # audible frequency to time mapping for video
    return 400 - (20 * (t - 16))


def freq2(t):
    # non audible frequency to time mapping
    return 40 - (4 * (t - 34))


def func(x, f, i):
    if f.get_value() < 108:
        radius = 3 - ((f.get_value() / (40 / f.get_value()) - i * 4) % 3)
        return x.become(
