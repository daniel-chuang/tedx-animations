...
class main(Scene):
    def construct(self):
        ...
        circle.add_updater(
            lambda x: x.become(Circle(radius.get_value(), z_index=5).move_to(origin.get_center()))
        )
        ...
        for i, sensor_group in enumerate(sensor_groups):
            ...
            sensor_group += Dot(sensor_group.get_center_of_mass(),
                                radius=0.4, color=GREY_B, z_index=2)
        ...
        for i, sensor_group in enumerate(sensor_groups):
            ...
            arrow = arrow.add_updater(
                lambda x: x.become(Arrow(sensor_groups[x.num].get_center_of_mass(), sensor_groups[x.num].get_center_of_mass() * (0.14 - 0.042*x.num) + origin.get_center_of_mass(), buff=0, color=GREEN_C, z_index=1))
            )
            arrows += arrow
        ...
        self.play(Write(text), run_time=0.5)
        self.wait(2.2)
        self.play(text.animate.become(Text("Moving the sound around...", z_index=20).shift(UP + RIGHT * 3.2).scale(0.75)))
        ...
        for arrow in arrows:
            ...
            arrow = arrow.add_updater(
                    lambda x: x.become(Line(sensor_groups[x.num].get_center_of_mass(), origin.get_center_of_mass(), color=GREEN_C, z_index=1))
            )
